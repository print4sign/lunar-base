<div
    x-data="onlineDesigner({
        config: @js($canvasConfig),
        design: @js($designs[$currentSide]),
        side: @js($currentSide),
    })"
    x-on:save-current-design.window="saveDesign()"
    x-on:load-design.window="loadDesign($event.detail.design)"
    class="online-designer"
>
    {{-- Front/Back Tabs (for frontback type) --}}
    @if($this->isFrontBack)
        <div class="flex items-center gap-2 mb-4 border-b border-gray-200 px-4">
            <button
                type="button"
                wire:click="switchSide('front')"
                @class([
    ...
    <div class="flex gap-4 px-4">
        {{-- Left Toolbar --}}
        <div class="w-12 flex flex-col gap-1 bg-gray-100 rounded-lg p-2 shrink-0">
            {{-- Select Tool --}}
            <button
                type="button"
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div
                    ...
                    x-transition
                    class="absolute left-full ml-2 top-0 bg-white shadow-lg rounded-lg p-1 z-20 min-w-[120px]"
                >
            ...
            {{-- Image Upload --}}
            <label class="p-2 rounded hover:bg-gray-200 transition-colors cursor-pointer" :title="'{{ __('online_designer.tools.image') }}'">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {{-- Canvas Area --}}
        <div class="flex-1 relative bg-gray-200 rounded-lg overflow-hidden" style="min-height: 500px;">
            {{-- Zone Legend --}}
            <div class="absolute top-3 left-3 bg-white/95 backdrop-blur rounded-lg px-3 py-2 text-xs z-10 shadow-sm">
            ...
            {{-- Dimensions Badge --}}
            <div class="absolute top-3 right-3 bg-white/95 backdrop-blur rounded-lg px-3 py-2 text-xs z-10 shadow-sm">
                <span class="font-medium">{{ $canvasConfig['widthMm'] }} x {{ $canvasConfig['heightMm'] }} mm</span>
                @if($this->hasBleed)
        ...
        <div class="w-56 flex flex-col gap-4 shrink-0">
            ...
            <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div class="px-3 py-2 bg-gray-50 border-b border-gray-200 font-medium text-sm">
                ...
                <div class="max-h-40 overflow-y-auto">
                    ...
                    <div x-show="layers.length === 0" class="px-3 py-4 text-sm text-gray-400 text-center italic">
            ...
            {{-- Properties Panel --}}
            <div x-show="hasSelection" x-transition class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div class="px-3 py-2 bg-gray-50 border-b border-gray-200 font-medium text-sm">
                ...
                <div class="p-3 space-y-3">
                    ...
                    <div>
                        ...
                        <input type="color" x-model="selectedProps.fill" x-on:input="updateSelectedProperty('fill', $event.target.value)" class="w-full h-8 rounded border border-gray-300 cursor-pointer" />
                    </div>

                    {{-- Stroke Color --}}
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">{{ __('online_designer.properties.stroke') }}</label>
                        <input type="color" x-model="selectedProps.stroke" x-on:input="updateSelectedProperty('stroke', $event.target.value)" class="w-full h-8 rounded border border-gray-300 cursor-pointer" />
                    </div>
                    ...
                    <template x-if="selectedProps.type === 'textbox' || selectedProps.type === 'i-text'">
                        <div class="space-y-3 pt-2 border-t border-gray-100">
                            <div>
                                ...
                                <input type="number" x-model.number="selectedProps.fontSize" x-on:input="updateSelectedProperty('fontSize', parseInt($event.target.value))" min="8" max="200" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm" />
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">{{ __('online_designer.properties.font_weight') }}</label>
                                <select x-model="selectedProps.fontWeight" x-on:change="updateSelectedProperty('fontWeight', $event.target.value)" class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm">
    ...
    <div class="mt-4 flex justify-between items-center px-4">
        <div class="text-sm text-gray-500">
            @if($this->isFrontBack)
                {{ __('online_designer.status.frontback', ['front' => !empty($designs['front']) ? '✓' : '○', 'back' => !empty($designs['back']) ? '✓' : '○']) }}
            @endif
        </div>
        <div class="flex gap-3">
            ...
            >
                {{ __('online_designer.actions.clear') }}
            </button>
            <button
                type="button"
                wire:click="confirmDesign"
                @disabled(!$this->hasDesign)
                @class([
                    'px-4 py-2 text-sm font-medium rounded-lg transition-colors',
                    'bg-primary-600 text-white hover:bg-primary-700' => $this->hasDesign,
                    'bg-gray-200 text-gray-400 cursor-not-allowed' => !$this->hasDesign,
                ])
            >
                {{ __('online_designer.actions.confirm') }}
            </button>
        </div>
    </div>
</div>
